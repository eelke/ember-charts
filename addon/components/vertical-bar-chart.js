...
export default ChartComponent.extend(LegendMixin, FloatingTooltipMixin, AxesMixin,
  FormattableMixin, SortableChartMixin, NoMarginChartMixin, {
    ...
    sortedData: Ember.computed('data.[]', 'sortKey', 'sortAscending', 'stackBars', function() {
      var data, group, groupData, groupObj, groupedData, key, newData, sortAscending, sortedGroups,
        summedGroupValues, _i, _len;
    ...
    hasLegend: Ember.computed('stackBars', 'isGrouped', 'legendItems.length', 'showLegend', function() {
      return this.get('stackBars') || this.get('isGrouped') && this.get('legendItems.length') > 1 && this.get(
        'showLegend');
    }),

    legendItems: Ember.computed('individualBarLabels.[]', 'getSeriesColor', 'stackBars', 'labelIDMapping.[]',
      function() {
        var getSeriesColor;
    ...
    groupAttrs: Ember.computed('graphicLeft', 'graphicTop', 'xBetweenGroupScale', function() {
      ...
      return {
        transform: (d) => {
          var dx = xBetweenGroupScale(d.group) ? this.get('graphicLeft') + xBetweenGroupScale(d.group) : this.get(
            'graphicLeft');
